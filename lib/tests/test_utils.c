...
main (int argc, char **argv)
{
	...
	setenv("NAGIOS_PLUGIN_STATE_DIRECTORY", "/usr/local/monitoring/var", 1);
	temp_string = (char *) _np_state_calculate_location_prefix();
	ok(!strcmp(temp_string, "/usr/local/monitoring/var"), "Got default directory" );
	...
	ok( !strcmp(temp_state_key->_filename, "/usr/local/monitoring/var/check_test/allowedchars_in_keyname"), "Got internal filename" );
	...
	*/
	...
	ok( !strcmp(temp_state_key->_filename, "/usr/local/monitoring/var/check_test/funnykeyname"), "Got internal filename" );
